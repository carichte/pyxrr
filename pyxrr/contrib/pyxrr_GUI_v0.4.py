...
# It is only working with pyxrr 0.9.06.
...
class MyFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.params = {'LayerCount': [1, 2, 1], 'N': [1, 1, 1], 'energy0': 8.04116, 'resolution0': 0.01, 'offset0': 0.0, 'scale0': 1.0, 'background0': -6.0, 'rho_0': 0.0012, 'rho_1': 4.0, 'rho_2': 2.0, 'rho_3': 2.336, 'd_0': 0.0, 'd_1': 100.0, 'd_2': 20.0, 'd_3': 0.0, 'sigma_0': 1.0, 'sigma_1': 1.0, 'sigma_2': 1.0, 'grad_d_0': 0.0, 'grad_d_1': 0.0, 'grad_d_2': 0.0}
    ...
    def create_menu(self):
        ...
        self.Bind(wx.EVT_MENU, self.on_open_model, m_loadmodel)
    ...
    def create_main_panel(self):
        ...
        self.b_load = wx.Button(self.panel, -1, "Datei laden...", size=(170,25))
        self.b_load.Bind(wx.EVT_BUTTON, self.on_open_file)
        
        self.b_loadmodel = wx.Button(self.panel, -1, "Modell laden...", size=(170,25))
        self.b_loadmodel.Bind(wx.EVT_BUTTON, self.on_open_model)
        
        self.b_savemodel = wx.Button(self.panel, -1, "Modell speichern...", size=(170,25))
        ...
        self.cb_angle = wx.ComboBox(self.panel, -1, choices=['q_z (A)', 'q_z (nm)', 'omega', '2theta'], style=wx.CB_READONLY, size=(102,-1))
        ...
        self.b_runfit = wx.Button(self.panel, -1, "Fit starten!", size=(160,35))
    ...
    def on_open_file(self, event):
        file_choices = "Daten-Typen (*.asc, *.fio, *.njc, *.raw, *.val, *.x00)|*.asc;*.fio;*.njc;*.raw;*.val;*.x00|ASC-Datei (*.asc)|*.asc|Hasylab (*.fio)|*.fio|Seifert (*.njc)|*.njc|Bruker (*.raw)|*.raw|Seifert (*.val)|*.udf|Philips (*.x00)|*.x00|TXT-Datei (*.txt)|*.txt|DAT-Datei (*.dat)|*.dat|Parameter-Datei (*.param)|*.param|Alle Dateien (*.*)|*.*"
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                wx.MessageBox('Fehler beim Laden der Datei:\n' + self.path + '\n\nFehlermeldung:\n' + str(error) + '\n\n(Pfad darf weder Umlaute noch Leerzeichen enthalten!)', 'Fehler', style=wx.ICON_ERROR)

        dlg.Destroy()
        
    def on_open_model(self, event):
        file_choices = "Parameter-Datei (*.param)|*.param"
        dlg = wx.FileDialog(self, "Modell laden", wildcard=file_choices, style=wx.OPEN)
    ...
    def open_model(self, openpath):
        ...
        if changed_range == 0:
            self.t_start.SetValue(str(0))
            self.t_end.SetValue(str(100)) 
        if changed_tt == 0:
            self.cb_angle.SetValue('omega')
        ...
        if self.sample.pol[0] == 0.0:
            self.cb_pol.SetValue('senkrecht')
        elif self.sample.pol[0] == 1.0:
            self.cb_pol.SetValue('parallel')
        elif self.sample.pol[0] == 0.5:
            self.cb_pol.SetValue('unpolarisiert')
    ...
    def make_model(self):
        string = []
        string.append("Ambience: name=Umgebung, code=" + self.model[0] + ", rho=" + str(self.params['rho_0']) + "\n")
        
        layer_pos = 0
        sigma_pos = 0
        for i in range(len(self.params['LayerCount']) - 2):
            string.append("\nGroup: name=Gruppe_" + str(i+1) + ", sigma=" + str(self.params['sigma_' + str(sigma_pos)]) + ", periods=" + str(self.params['N'][i+1]) + ", grad_d=0\n")
            sigma_pos += 1
            multilayer_sigma = 0
            for j in range(self.params['LayerCount'][i+1]):
                layer_pos += 1
                if j == 0:
                    if self.params['N'][i+1] == 1:
                        string.append("Layer: name=Schicht_" + str(layer_pos) + ", code=" + self.model[layer_pos] + ", rho=" + str(self.params['rho_' + str(layer_pos)]) + ", d=" + str(self.params['d_' + str(layer_pos)]) + "\n")
                    else:
                        special_sigma = sigma_pos + self.params['LayerCount'][i+1] - 1
                        multilayer_sigma = 1
                        string.append("Layer: name=Schicht_" + str(layer_pos) + ", code=" + self.model[layer_pos] + ", rho=" + str(self.params['rho_' + str(layer_pos)]) + ", d=" + str(self.params['d_' + str(layer_pos)]) + ", sigma=" + str(self.params['sigma_' + str(special_sigma)]) + "\n")
                else:    
                    string.append("Layer: name=Schicht_" + str(layer_pos) + ", code=" + self.model[layer_pos] + ", rho=" + str(self.params['rho_' + str(layer_pos)]) + ", d=" + str(self.params['d_' + str(layer_pos)]) + ", sigma=" + str(self.params['sigma_' + str(sigma_pos)]) + "\n")
                    sigma_pos += 1
            sigma_pos += multilayer_sigma
        ...
        if value == 'q_z (A)':
            ...
            x_axis_value = 'twotheta'
        else:
            x_axis_value = 'theta'
        
        string.append("\nSubstrate: name=Substrat, code=" + self.model[-1] + ", rho=" + str(self.params['rho_' + str(layer_pos+1)]) + ", sigma=" + str(self.params['sigma_' + str(sigma_pos)]) + "\n")          
        string.append("\nMeasurement: file=" + self.path + ", x_axis=" + x_axis_value + ", fit_range=" + self.t_start.GetValue() + "->" + self.t_end.GetValue() + ", energy=" + str(self.params['energy0']) + ", resolution=" + str(self.params['resolution0']) + ", offset=" + str(self.params['offset0']) + ", scale=" + str(self.params['scale0']) + ", background=" + str(self.params['background0']) + ", pol=" + pol + "\n")
    ...
    def check_float(self, event):
        ...
        self.on_change_measparams(event)
    ...
    def on_run_fit(self, event):
        ...
        if fit_keys == []:
            ...
            try:
                fitted_param = self.sample.fit(var_names=fit_keys)
